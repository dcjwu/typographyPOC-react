      ...
      <>
         {
            !isDataLoaded
               ...
               : orderListFromDb && orderListFromDb.length === 0
                  ? <>
                     <h1 className="alert-warning p-5 text-center">No orders to
                        display...</h1>
                        ...
                        <div className="admin-filters">
                           {
                              filterButtons.map(button => {
                                 if (button === "in progress") return <Button key={button}
                                    additionalClass="text-capitalize btn-outline-warning"
                                    onClick={() => handleFilteredProducts(button)}>{button}</Button>
                                 if (button === "completed") return <Button key={button}
                                    additionalClass="text-capitalize btn-outline-success"
                                    onClick={() => handleFilteredProducts(button)}>{button}</Button>
                                 if (button === "canceled") return <Button key={button}
                                    additionalClass="text-capitalize btn-outline-danger"
                                    onClick={() => handleFilteredProducts(button)}>{button}</Button>
                                 if (button === "all") return <Button key={button}
                                                                      additionalClass="text-capitalize"
